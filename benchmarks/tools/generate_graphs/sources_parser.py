...
def parse_sources_from_json(j: dict, config_file_path: str,
	...
	for input in input_file_paths:
		src_info: visualize.source_info = visualize.source_info(input, "", True)
	...
	if jdefault_scale:
		...
		if jtype == "relative":
			...
			info.default_scale = visualize.scaling_info(visualize.graph_scaling.relative, jto)
		else:
			info.default_scale = visualize.scaling_info(visualize.graph_scaling.absolute, "")
		...
		for jcategory in jcategories:
			...
			if jcatscale is not None:
				...
				if jtype == "relative":
					...
					scale = visualize.scaling_info(visualize.graph_scaling.relative,
					                               jcatscaletype)
				else:
					scale = visualize.scaling_info(visualize.graph_scaling.absolute, "")
			else:
				scale = info.default_scale
			cat_info: visualize.category_info = visualize.category_info(
			    name, scale, order, jcatpattern, jcatexclude)
	...
	if len(info.data_labels) < 1:
		info.data_labels = [
		    visualize.data_label_info("real_time", visualize.data_label_format.clock, True),
