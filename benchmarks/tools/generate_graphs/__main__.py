...
def ordinal(value) -> str:
	...
	return '%d%s' % (value, "tsnrhtdd"[(value // 10 % 10 != 1) * (value % 10 < 4) * value % 10::4])
...
def draw_graph(benchmark: visualize.benchmark):
	...
	group_names = [label.name for label in benchmark.groups]
	bars: List[Union[matplotlib.pyplot.Text, matplotlib.container.BarContainer]] = []
	scatters: List[matplotlib.collections.PathCollection] = []
	num_groups: int = len(benchmark.groups)
	num_data_labels: int = len(benchmark.analysis_info.data_labels)
	index_num_data_labels: int = num_data_labels - 1
	bar_padding = 0.05
	group_padding = 0.15
	bar_height = 0.50
	group_height = (bar_height * num_data_labels) + (bar_padding *
	                                                 index_num_data_labels) + group_padding
	...
	for group_index, group in enumerate(benchmark.groups):
		color_index: int = group.name_index % len(data_point_aesthetics)
		aesthetics: Tuple[str, Union[str, None]] = data_point_aesthetics[color_index]
		...
		for label_index, _ in enumerate(group.labels):
			...
			label_bar_color = matplotlib.colors.hsv_to_rgb(label_bar_color_hsv)
			label_edge_color_hsv = matplotlib.colors.rgb_to_hsv(
			    matplotlib.colors.hex2color(primary_bar_color))
			label_edge_color_hsv[2] *= (0.6 - (0.1 * label_index))
			label_edge_color = matplotlib.colors.hsv_to_rgb(label_bar_color_hsv)
		...
		for label_index, label_name in enumerate(group.labels):
			...
			bar_y = (group_index * group_height) + (label_index * (bar_height + bar_padding))
			...
			bar: matplotlib.container.BarContainer = axes.barh(bar_y,
			                                                   mean,
			                                                   height=bar_height,
			                                                   xerr=stddev,
			                                                   linewidth=0.2,
			...
			yscatter = [
			    bar_y + random.uniform(quarter_bar_height, bar_height - quarter_bar_height)
			    for _ in xscatter
			]
			scatter_alpha = 0.20 if xscatter_len < 11 else 0.10 if xscatter_len < 101 else 0.05 if xscatter_len < 1001 else 0.002
			scatter: matplotlib.collections.PathCollection = axes.scatter(xscatter,
			                                                              yscatter,
			                                                              color=bar_color,
			                                                              edgecolor='#000000',
			                                                              linewidth=0.5,
	...
	xscale_bisect_index: int = bisect.bisect_left(label_info.format_list,
	                                              benchmark_max,
	...
	axes.set_xticks(numpy.arange(0, xlimit, adjusted_xlimit / 10.0))
	axes.xaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(time_axis_formatting))

	# have ticks drawn from base of bar graph
	# to text labels
	y_tick_height = group_height
	y_ticks = [((y_index + 0.45) * y_tick_height) for y_index, _ in enumerate(group_names)]
	y_limits = [
	    bar_y_positions[0] - group_padding, bar_y_positions[-1] + bar_height + group_padding
	]
	...
	data_label_descriptions: List[str] = [
	    ordinal(label_index + 1) + " is " + label_info.name
	    for label_index, label_info in enumerate(benchmark.analysis_info.data_labels[::-1])
	]
...
def main():
	...
	parser = argparse.ArgumentParser(
	    description='Generate graphs from a json listing of data in various formats...')
	parser.add_argument('-c',
	                    '--config',
	                    nargs='?',
	                    default='graph_config.json',
	                    type=argparse.FileType('r'))
	parser.add_argument('-i', '--inputs', nargs='*', default=['benchmarks.json'])
	...
	benchmarks: List[visualize.benchmark] = benchmarks_parser.parse_benchmarks_from_sources(info)
