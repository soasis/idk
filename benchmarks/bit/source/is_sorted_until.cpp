...
static void is_sorted_until_naive(benchmark::State& state) {
...
static void is_sorted_until_naive_packed(benchmark::State& state) {
...
static void is_sorted_until_ztdc_packed(benchmark::State& state) {
	constexpr std::size_t size_bits = sizeof(std::size_t) * CHAR_BIT;
	using C                         = std::array<std::size_t, (100032 + size_bits - 1) / (size_bits)>;
	C c;
	c.fill(0);
	...
	for (auto _ : state) {
		...
		for (; it != end_it; ++it) {
			...
			if (!seen_true) {
				...
				if (target_word != (std::numeric_limits<std::size_t>::max)()) {
					break;
				}
			}
		}
...
static void is_sorted_until_cpp_std_array_bool(benchmark::State& state) {
	using C = std::array<bool, 100032>;
	C c;
	c.fill(opaque_false());
...
static void is_sorted_until_cpp_std_vector_bool(benchmark::State& state) {
...
static void is_sorted_until_cpp_std_bitset(benchmark::State& state) {
...
BENCHMARK(is_sorted_until_naive);
BENCHMARK(is_sorted_until_naive_packed);
BENCHMARK(is_sorted_until_ztdc_packed);
BENCHMARK(is_sorted_until_cpp_std_array_bool);
BENCHMARK(is_sorted_until_cpp_std_vector_bool);
BENCHMARK(is_sorted_until_cpp_std_bitset);
